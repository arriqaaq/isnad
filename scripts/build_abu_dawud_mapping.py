#!/usr/bin/env python3
"""
Build Abu Dawud hadith_number → page_index mapping for Awn al-Ma'bud.

Two methods for 100% coverage:
  1. Sequential باب alignment with Abu Dawud chapters (~5132 hadiths)
  2. Interpolation from nearest preceding mapped hadith (fills gaps)

Requires:
  data/awn_mabud_headings.json  (from fetch_awn_mabud.py)
  data/semantic_hadith.json     (source hadith data)

Outputs:
  data/awn_mabud_hadith_mapping.json
"""
...
MAPPING_FILE = os.path.join(DATA_DIR, "awn_mabud_hadith_mapping.json")

# Abu Dawud is book 3 in BOOK_ORDER (SB=1, SM=2, SD=3, JT=4, SN=5, IM=6)
BOOK_PREFIX = "SD"


def main():
    for path, label in [
        (HEADINGS_FILE, "Awn al-Ma'bud headings"),
    ...
    print(f"\nSpot checks:")
    for h_num in [1, 2, 50, 100, 500, 1000, 3000, 5000, 5200]:
