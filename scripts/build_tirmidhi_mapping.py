#!/usr/bin/env python3
"""
Build Tirmidhi hadith_number → page_index mapping for Tuhfat al-Ahwadhi.

Two methods for 100% coverage:
  1. Sequential باب alignment with Tirmidhi chapters (~2715 hadiths)
  2. Interpolation from nearest preceding mapped hadith (fills gaps)

Requires:
  data/tuhfat_ahwadhi_headings.json  (from fetch_tuhfat_ahwadhi.py)
  data/semantic_hadith.json          (source hadith data)

Outputs:
  data/tuhfat_ahwadhi_hadith_mapping.json
"""
...
MAPPING_FILE = os.path.join(DATA_DIR, "tuhfat_ahwadhi_hadith_mapping.json")

# Tirmidhi is book 4 in BOOK_ORDER (SB=1, SM=2, AD=3, JT=4, AN=5, IM=6)
BOOK_PREFIX = "JT"


def main():
    for path, label in [
        (HEADINGS_FILE, "Tuhfat al-Ahwadhi headings"),
    ...
    print(f"\nSpot checks:")
    for h_num in [1, 2, 50, 100, 500, 1000, 2000, 3000, 3900]:
