...
Usage:
  ...
  python3 scripts/fetch_tahdhib.py --pages     # headings + all 13120 pages (~25 min)

Outputs:
  data/tahdhib_headings.json  — book metadata + headings (9459 headings)
...
def fetch_book_metadata():
    ...
    print(f"Fetching book metadata for Tahdhib al-Tahdhib (book_id={BOOK_ID})...")
...
def fetch_all_pages(metadata: dict):
    ...
    print(f"Fetching all {total} pages for Tahdhib al-Tahdhib...")
...
def main():
    ...
    if "--pages" in sys.argv:
        ...
        page_map = indexes.get("page_map", [])
        print(f"\nSkipping page fetch. Use --pages to fetch all {len(page_map)} pages (~25 min).")
