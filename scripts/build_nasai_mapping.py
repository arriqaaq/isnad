#!/usr/bin/env python3
"""
Build Nasa'i hadith_number → page_index mapping for Sahih Sunan al-Nasa'i.

Two methods for 100% coverage:
  1. Sequential باب alignment with Nasa'i chapters (~5289 hadiths)
  2. Interpolation from nearest preceding mapped hadith (fills gaps)

Requires:
  data/sahih_nasai_headings.json  (from fetch_sahih_nasai.py)
  data/semantic_hadith.json       (source hadith data)

Outputs:
  data/sahih_nasai_hadith_mapping.json
"""
...
MAPPING_FILE = os.path.join(DATA_DIR, "sahih_nasai_hadith_mapping.json")

# Nasa'i is book 5 in BOOK_ORDER (SB=1, SM=2, SD=3, JT=4, SN=5, IM=6)
BOOK_PREFIX = "SN"


def main():
    for path, label in [
        (HEADINGS_FILE, "Sahih Nasai headings"),
    ...
    print(f"\nSpot checks:")
    for h_num in [1, 2, 50, 100, 500, 1000, 3000, 5000, 5700]:
