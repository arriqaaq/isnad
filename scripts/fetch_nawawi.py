...
Usage:
  ...
  python3 scripts/fetch_nawawi.py --pages     # headings + all 4086 pages (~8 min)

Outputs:
  data/nawawi_on_muslim_headings.json  — book metadata + headings (1145 headings)
...
def fetch_book_metadata():
    ...
    print(f"Fetching book metadata for Sharh Nawawi (book_id={BOOK_ID})...")
...
def fetch_all_pages(metadata: dict):
    ...
    print(f"Fetching all {total} pages for Sharh Nawawi...")
...
def main():
    ...
    if "--pages" in sys.argv:
        ...
        page_map = indexes.get("page_map", [])
        print(f"\nSkipping page fetch. Use --pages to fetch all {len(page_map)} pages (~8 min).")
