...
Usage:
  ...
  python3 scripts/fetch_sahih_nasai.py --pages     # headings + all 1168 pages (~2 min)

Outputs:
  data/sahih_nasai_headings.json  — book metadata + headings (2580 headings)
...
def fetch_book_metadata():
    ...
    print(f"Fetching book metadata for Sahih Sunan al-Nasa'i (book_id={BOOK_ID})...")
...
def fetch_all_pages(metadata: dict):
    ...
    print(f"Fetching all {total} pages for Sahih Sunan al-Nasa'i...")
...
def main():
    ...
    if "--pages" in sys.argv:
        ...
        page_map = indexes.get("page_map", [])
        print(f"\nSkipping page fetch. Use --pages to fetch all {len(page_map)} pages (~2 min).")
