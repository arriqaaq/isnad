...
Usage:
  ...
  python3 scripts/fetch_fathulbari.py --pages     # headings + all 7996 pages (~15 min)

Outputs:
  data/fath_al_bari_headings.json  — book metadata + headings (5110 headings)
...
def fetch_book_metadata():
    ...
    print(f"Fetching book metadata for Fath al-Bari (book_id={BOOK_ID})...")
...
def fetch_all_pages(metadata: dict):
    ...
    print(f"Fetching all {total} pages for Fath al-Bari...")
...
def main():
    ...
    if "--pages" in sys.argv:
        ...
        page_map = indexes.get("page_map", [])
        print(f"\nSkipping page fetch. Use --pages to fetch all {len(page_map)} pages (~15 min).")
