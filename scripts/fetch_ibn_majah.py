...
Usage:
  ...
  python3 scripts/fetch_ibn_majah.py --pages     # headings + all 3023 pages (~5 min)

Outputs:
  data/ibn_majah_headings.json  — book metadata + headings (1603 headings)
...
def fetch_book_metadata():
    ...
    print(f"Fetching book metadata for Sunan Ibn Majah (book_id={BOOK_ID})...")
...
def fetch_all_pages(metadata: dict):
    ...
    print(f"Fetching all {total} pages for Sunan Ibn Majah...")
...
def main():
    ...
    if "--pages" in sys.argv:
        ...
        page_map = indexes.get("page_map", [])
        print(f"\nSkipping page fetch. Use --pages to fetch all {len(page_map)} pages (~5 min).")
